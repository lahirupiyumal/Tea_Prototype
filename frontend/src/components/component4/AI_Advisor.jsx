			...
			<div className="advisor-layout">
				<section className="advisor-chat card">
					<div className="advisor-chat-header">
						...
						<div><strong>TeaNex AI</strong><span>Export intelligence assistant</span></div>
					...
					<div className="advisor-messages" aria-live="polite">
						{messages.map((message, index) => <div className={`advisor-message ${message.role}`} key={`${message.role}-${index}`}><div className="message-icon">{message.role === "assistant" ? <Bot size={14} /> : <User size={14} />}</div><div><span>{message.role === "assistant" ? "TeaNex AI" : "You"}</span><p>{message.text}</p></div></div>)}
					</div>
